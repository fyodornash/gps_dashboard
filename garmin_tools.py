...
@mongo_decorator
def get_TSSes(user_id=None):
    '''returns a list of list('TSS',str(datetime)) lists'''
    sort = {'$sort': {'time': 1}}
    rs = list(db.runsy.aggregate([{'$match': {'user_id': user_id}}, sort]))
...
@mongo_decorator
def get_training_summary(db=None, user_id=None):
    '''returns a list of ('TSS',str(datetime),duration,distance) dicts'''
    project = {'$project': {'df': {'$slice': ['$df', -1]}, 'TSS': 1, 'time': 1}}
    match = {'$match': {'user_id': user_id}}
    sort = {'$sort': {'time': 1}}
    test = list(db.runsy.aggregate([match, project, sort]))
